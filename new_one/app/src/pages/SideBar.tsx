...
function SideBar({ labelAdded, messageIdForTaskFn, updateLabelAdded, mailSubject, mailUrl }: any) {
  ...
  const [fetchedTasks, setFetchedTasks] = useState<Tasks[]>([]); //fetching tasks
  ...
  const [singleCompleted, setSingleCompleted] = useState<any[]>([]);
  ...
  const fetchCompletedCatchup = async () => {
    try {
      // setSingleLoading(true);
      let response = await fetch(`${TESTING_URL}/api/v1/tasks/tasks-type-and-standard-email-id?standardEmailId=${messageIdForTaskFn}&masterTaskTypeId=4`, {
        "method": "GET",
        "headers": {
          "Content-Type": "application/json",
          'Authorization': LocalStorageItem()
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setFetchedTasks(prevTasks => prevTasks.filter(task => task.id !== deletingTaskId));
      ...
      setButtonLoading(false);
      handleSnackbar("Failed to delete task", "error");
  ...
  const handleTaskComplete2 = async (taskId: number, progress: number) => {
    console.log("ss", taskId, progress);
    try {
      // setLoading(true);
      const response = await fetch(`${TESTING_URL}/api/v1/tasks/progress?taskId=${taskId}&progress=${progress}`, {
      ...
      const res = await response.json();
      if (res) {
        console.log('Task progress updated successfully2', res);
        setActiveTaskId(null);
        setLoading(false);
        if (progress === 100) {
        ...
        setLoading(false);
        console.error('Failed to update task progress2:', response.statusText);
      }
    } catch (error) {
      setLoading(false);
      console.error('Error updating task progress2:', error);
    }
  };
  ...
  const handleStatusChange2 = (taskId: any, selectedStatus: any) => {
    ...
    if (selectedStatus === "completed" || selectedStatus === "COMPLETED") {
      handleTaskComplete2(taskId, 100);
    }
    else {
  ...
  const handleChangeToInProgress2 = async (taskId: any) => {
    try {
      ...
      if (res) {
        ...
        // handleSnackbar("Back to Progress", "success");

        setFetchedCompletedTasks(prevCompletedTasks => prevCompletedTasks?.filter((task) => task.id !== taskId));
        setFetchedTasks((prevTask) => [...prevTask, res]);
        ...
        handleSnackbar("Failed to convert into Progress", "error");
  ...
  const handleCheckBox2 = async (taskId: number) => {
    console.log("taskId ignore", taskId);
    handleSnackbar("Task already completed!!", "success");
    // try {
    //   // setLoading(true);
    ...
    //     // fetchingTasks();
    //     // fetchAllCompletedTasks();
    //     setFetchedTasks((prevTasks)=> prevTasks?.filter((eachTask)=>eachTask.id !== taskId));
    //     setSelectedStatusMap2((prevMap) => ({
    //       ...prevMap,
    //       [taskId]: res.taskStatus.taskStatusName
    //     }));
    //     setFetchedCompletedTasks((prevCompletedTasks) => [...prevCompletedTasks, res]);
  ...
  const handleClickOfCheckbox = async () => {
    ...
    let data = {
      ...
      taskStatus: { id: 3 },
      client: { id: 1 },
      standardEmailId: messageIdForTaskFn
    }
    try {
      // setSingleLoading(true);
      ...
      let res = await response.json();
      // fetchCompletedCatchup();
      setSingleCompleted((prevTask) => [...prevTask, res]);
      setSingleLoading(false);
      console.log("data", res);
      handleSnackbar("AAdded to catchup", "success");
    }
    catch (err) {
      setSingleLoading(false);
      handleSnackbar("Unable to add to catchup", "error");
      console.error("err at handleClickOfcheckbox", err);
    }
  }

  const handleClickOfCheckboxRemove = async (taskId: any) => {
    // setSingleLoading(true);
    try {
      const response = await fetch(`${TESTING_URL}/api/v1/tasks?taskId=${taskId}`, {
        method: "DELETE",
        headers: {
          'Authorization': LocalStorageItem()
        }
      ...
      // fetchAllCompletedTasks();
      // fetchCompletedCatchup();
      setSingleCompleted((prevTasks) => prevTasks.filter(task => task.id !== taskId));
      handleCloseDeletePopover();
      handleSnackbar("Back to catchup", "success");
      setTimeout(() => {
      }, 1000)
      // setSingleLoading(false);
    }
      ...
      setSingleLoading(false);
      handleSnackbar("Failed to complete catchup", "error");
      console.log("error deleting task", err);
    }
  }
  ...
  const handleNewTaskAdded = (newTask: Tasks) => {
    setFetchedTasks((prevTasks) => [...prevTasks, newTask]);
    setSelectedStatusMap((prevMap) => ({
      ...prevMap,
      [newTask.id]: newTask.taskStatus.taskStatusName || "UndefinedStatus",
    }));
  };
              ...
              (
                <>
...

                  {
                    singleLoading ? (
                      <>
                        <Skeleton animation="wave" width={280} height={100} style={{ marginLeft: "10px", marginTop: "-10px", marginBottom: "-10px" }} />

                      </>
                    ) :

                      labelAdded && !singleCompleted.length && (
                        ...
                        fetchedTasks && fetchedTasks.length > 0 ? (
                          <main>

                            {
                              ...
                              <NewTaskForm messageIdForTaskFn={messageIdForTaskFn} mailSubject={mailSubject} closeTaskForm={closeTaskFormFn} toRender={toRenderFn} mailUrl={mailUrl} handleNewTaskAdded={handleNewTaskAdded} />
                            }
                                  ...
                                  fetchedTasks?.map((ele) => {
                                    ...
                                    return <div className={`taskOuterDiv`} key={ele.id} onMouseEnter={() => { handleTaskCardHover(ele.id) }} onMouseLeave={handleTaskCardUnhover}>
                                      ...
                                      {editTaskId?.id == ele.id ? (
                                        <NewTaskEdit task={editTaskId} closeTaskFormEdit={closeTaskFormEdit} submitTaskFormEdit={submitTaskFormEdit} setFetchedTasks={setFetchedTasks} />
                                      ) : (
                                        <>
                                              ...
                                              <Select className="selectTaskTypee"
                                                ...
                                                renderValue={(selected) => (
                                                  (selected === 'Todo' || selected === 'TODO') ? (
                                                    ...
                                                    (selected === 'inProgress' || selected === 'IN PROGRESS' || selected === 'In Progress') ? (
                                                      <span className="imageOfStatus">
                          ...
                          : (
                            ...
                            labelAdded ?
                              ...
                              <NewTaskForm messageIdForTaskFn={messageIdForTaskFn} mailSubject={mailSubject} toRender={toRenderFn} mailUrl={mailUrl} handleNewTaskAdded={handleNewTaskAdded} />
                          )
                      ...
                      showCompletedList && (
                        <div id="iii">
                          ...
                          {singleCompleted.length > 0 && (
                            singleCompleted.map((ele, index) => (
                              <div className="catchupShowInitiallyDiv" key={index}>
                                <div>
                                  <Checkbox color="success" checked size="small" onClick={() => handleClickOfCheckboxRemove(ele.id)} />
                                </div>
                                <div>
                                  <p>{mailSubject}</p>
                                  <div className="catchupShowInitiallyInnerDiv">
                                      ...
                                      <div className="taskWrapper">
                                        ...
                                        <div className="checkboxDivSB">
                                          <Tooltip title="In progress">
                                            <Checkbox size="small" className="checkboxSB" color="success" checked onClick={() => { handleChangeToInProgress2(ele.id) }} />
                                          </Tooltip>
                                        </div>
