...
const OuterSidebar = () => {
    ...
    util.create_window_api({
      "mail_opened": async ({ mailDetail }: { mailDetail: any }) => {
        if (!isMailOpenedRef.current) {
          ...
          // const currentEmailId = mailDetail.messageId;
          // let openedEmailIds = JSON.parse(localStorage.getItem('openedEmailIds') || '[]');
          // console.log("openedEmailIds", openedEmailIds);
          // if (!openedEmailIds.includes(currentEmailId)) {
            //   openedEmailIds.push(currentEmailId);
            //   localStorage.setItem('openedEmailIds', JSON.stringify(openedEmailIds));
            
            // console.log("openedEmailIds", openedEmailIds);
            // }


            const lastInvocationTimestamp = localStorage.getItem('lastOpenNudgeInvocation');
            const now = new Date().getTime(); // Get the current timestamp
            // Check if it's been more than 24 hours since the last invocation
            if (!lastInvocationTimestamp || now - parseInt(lastInvocationTimestamp) > 24 * 60 * 60 * 1000) {
              window_send_message("parent", "openNudge");
              // Update the last invocation timestamp in localStorage
              localStorage.setItem('lastOpenNudgeInvocation', now.toString());
      ...
      "header_button_clicked": async ({ mailDetailHBC }: { mailDetailHBC: any }) => {
        console.log("got header button clicked");
        return handleCatchupLater1(mailDetailHBC);
      },
    });
  }, []);
  ...
  //   if (extensionData) {
  //     setEmailAddressGlobal(extensionData.emailAddress);
  //     localStorage.setItem(`${extensionData.emailAddress}`, extensionData.accessToken);
  //     Cookies.set(`${extensionData.emailAddress}`, extensionData.accessToken);
  //   }
  // }
  // console.log("emailAddressGlobal",emailAddressGlobal );
  ...
  const handleClose = () => {
    ...
    window_send_message("parent", "handle_outer_sidebar_close_fully");
  }
  ...
  async function checkIfLabelIsApplied1(messageId: string) {
    ...
    try {
      ...
      let result = await res.json();
      console.log("res1 checkIflabelApplied", result);
      if (result.catchUpLater) {
        return true;
      }
      else if (result.message) {
      ...
      console.log("error1 at Outersidebar checking label", err);
      setIsLoading(false);
      return false;
    }
  ...
  async function handleCatchupLater1(mailDeatailHBC: any) {
    let result = await checkIfLabelIsApplied1(mailDeatailHBC.messageId);
    if (result) {
      return result;
    }
    else {
      ...
      try {
        ...
        const response = await fetch(`${TESTING_URL}/api/v1/emails/catch-up-later-emails`, {
          method: "POST",
          body: JSON.stringify({ standardEmailId: mailDeatailHBC.messageId, title: mailDeatailHBC.subject, createdDate: formattedDate, mailUrl: mailDeatailHBC.singleMailUrl }),
          headers: {
            'Content-Type': 'application/json',
            'Authorization': LocalStorageItem()
          }
  ...
  async function handleCatchupLater() {
    ...
    setButtonLoading(true);
    console.log("msgid", messageIdForTask, mailSubject)
    try {
      const formattedDate = formatCustomDate();
      const response = await fetch(`${TESTING_URL}/api/v1/emails/catch-up-later-emails`, {
        method: "POST",
